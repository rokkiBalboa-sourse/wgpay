...
async def Work_with_Message(m: types.Message):
    ...
    if e.demojize(m.text) == "Как подключить :gear:":
        if user_dat.trial_subscription == False:
            ...
            Butt_how_to.add(
                types.InlineKeyboardButton(e.emojize("Подробная инструкция"),
                                           url="https://telegra.ph/Gajd-na-ustanovku-11-27"))
            Butt_how_to.add(
                types.InlineKeyboardButton(e.emojize("Проверить доступ"),
                                           url="https://2ip.ru/"))
...
async def Buy_month(call: types.CallbackQuery):
    ...
    if payment_info is None:
        ...
        if(Month_count == 6):
            count = CONFIG['perc_6']
        bill = await bot.send_invoice(call.message.chat.id, f"Оплата доступа", f"доступ на {str(Month_count)} мес. Выгода {round(((Month_count - count) / Month_count) * 100)}%", call.data,
                                        currency="RUB",prices=[
                    types.LabeledPrice(f"Доступ на {str(Month_count)} мес.  Выгода {round(((Month_count - count) / Month_count) * 100)}%", round(count * CONFIG['one_month_cost'] * 100))],
